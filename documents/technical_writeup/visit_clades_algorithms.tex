...
\toprule
Method & Prepare & Per-call & Memory \\
\midrule
Dense        & $O(nm)$  & $O(nm)$ & $O(nm)$ \\
Tree shuttle & $O(nm)$  & $O(n \cdot n_i + M)$ & $O(m \cdot d)$ peak \\
RLE          & $O(nm)$  & $O(m + R)$ & $O(R)$ \\
\bottomrule
\end{tabular}
\caption{Complexity comparison.  $n_i$: number of global intervals; $M$:
total mismatches; $R$: total 1-runs; $d$: threading tree depth.  Tree
shuttle replaces the $O(m)$ prefix-sum sweep of RLE with $O(n \cdot n_i)$
tree propagation, where $n_i \ll m$ typically.  RLE's per-call work is
embarrassingly parallel; tree shuttle has sequential dependencies in the
cumulative-sum propagation for right multiply and in Pass~1 of left
multiply.}
