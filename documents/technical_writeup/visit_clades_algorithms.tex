...
\textbf{Batch multiply} ($k$ vectors simultaneously):
Prefix sums, cumulative rows, $W$ vectors, and $\delta$ arrays are widened
to $k$ columns.  Inner loops over $k$ elements auto-vectorize via SIMD.
The batch left multiply uses precomputed ancestor chains (not
per-query binary search) to keep the inner loop tight:
for each ancestor step, a length-$k$ vector subtraction (detach) or
addition (attach) is performed.

\textbf{Range multiply} (submatrix $\mathbf{G}[s_a{:}s_b, :]$):
Restricts interval iteration to $[j_a, j_b)$ where $j_a, j_b$ are the
intervals containing $s_a, s_b$.  The full precomputed tree structure is
used (correct carry state), but only sites in $[s_a, s_b)$ contribute to
the output.  Input vector has length $s_b - s_a$ (right) or $n$ (left).
