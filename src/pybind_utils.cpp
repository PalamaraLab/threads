...
py::tuple threading_instructions_get_state(const ThreadingInstructions& ti) {
    // Instructions may be stored as a 2-tuple with first arg being a list
    // of per-instruction data, and the second being the positions.
    py::list inst_list;
    ...
    return py::make_tuple(inst_list, ti.positions);
}

ThreadingInstructions threading_instructions_set_state(py::tuple tup) {
    // Unpack the 2-tuple into threading instructions and positions
